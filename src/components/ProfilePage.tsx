...
const ProfilePage: React.FC<ProfileProps> = ({ user, onLogout }) => {
  ...
  const fetchPreferences = () => {
    axios
      .get(`/api/getStablePreferences?email=${encodeURIComponent(user.email)}`)
      .then((response) => {
  ...
  const handleAvailabilityChange = (day: string, time: string) => {
    setAvailability((prev) => ({
      ...
      [day]: {
        ...prev[day],
        [time]: !(prev ? prev[day][time] : false), // Toggle the value
      },
    }));
  };
  const handleSubmission = () => {
    axios
      .post("/api/setStablePreferences", {
        email: user.email,
                    ...
                    {timeSlots.map((time) => (
                      <label key={time}>
                        <input
                          type="checkbox"
                          checked={
                            availability ? availability[day][time] : false
                          }
